...
CREATE TABLE IF NOT EXISTS `orchestrator`.`templates` (
  ...
  `status` VARCHAR(40) NULL DEFAULT 'draft',
  ...
  INDEX `fk_templates_owner` (`owner_id` ASC) VISIBLE,
  INDEX `fk_templates_az` (`az_id` ASC) VISIBLE,
  INDEX `fk_templates_created_by` (`created_by` ASC) VISIBLE,
  INDEX `fk_templates_updated_by` (`updated_by` ASC) VISIBLE,
  INDEX `fk_templates_deleted_by` (`deleted_by` ASC) VISIBLE,
  CONSTRAINT `fk_templates_az`
    FOREIGN KEY (`az_id`)
  ...
  CONSTRAINT `fk_templates_created_by`
    ...
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `fk_templates_deleted_by`
    FOREIGN KEY (`deleted_by`)
    REFERENCES `orchestrator`.`users` (`user_id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
